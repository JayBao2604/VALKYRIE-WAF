...
class PayloadGenerator:
    ...
    def verify_generation(self, num_tests: int = 100) -> bool:
        ...
        return True
    dir: str = '.') -> Tuple[List[str], List[List[int]]]:
        """
        ...
        # Define output files
        output_file = f"{output_dir}/{self.attack_type}.txt"
        tokenized_file = f"{output_dir}/tokenized_{self.attack_type}.json"
        vocab_file = f"{output_dir}/{self.attack_type}_vocab.json"
        ...
        print(f"✅ Successfully generated {len(payload_set)} unique {self.attack_type.upper()}
        
        print(f"✅ Successfully generated {len(sqli_set)} unique payloads")
        print(f"📊 Vocabulary size: {len(self.vocab)} tokens")
    generate_all_attack_payloads(num_payloads: int = 15000, output_dir: str = 'generated_payloads'):
    """
    ...
    attack_types = {
        'sqli': 'grammars/sqli.txt',
        'xss': 'grammars/xss.txt',
        'rce': 'grammars/rce.txt',
        'nosqli': 'grammars/nosqli.txt',
        'ssrf': 'grammars/ssrf.txt'
    }
...
def main():
    ...
    if args.attack_type == 'all':
        ...
        generator.verify_generation(num_tests=100)
        generator.generate_payloads(num_payloads=args.num_payloads, output_dir=args.output_dir    num_payloads=15000,
        output_file='sqli.txt',
        tokenized_file='tokenized_sqli.json',
