...
def normalize_payload(payload: str) -> str:
    payload = re.sub(r"(%27|')\+?(%27|')+", "%27", payload)
...
def detect_attack_root_symbol(payload: str) -> str:
    ...
    if "sleep" in lower or "benchmark" in lower:
        return "piggyAttack"
    if "extractvalue" in lower or "updatexml" in lower:
        return "errorAttack"
    if re.search(r"\b(or|and|\|\||&&)\b", lower):
...
def refine_if_needed(original_payload: str,
    ...
    if grammar_file is None:
        grammar_file = base_dir / "sqli" / "sqli_grammar.txt"
    if vocab_file is None:
        vocab_file = base_dir / "sqli" / "vocab.json"
