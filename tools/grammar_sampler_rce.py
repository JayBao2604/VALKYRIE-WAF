...
# ========== ROOT DETECTOR FOR RCE ==========
def detect_attack_root_symbol(payload: str) -> str:
    lower = payload.lower()
    if any(cmd in lower for cmd in ['ps', 'whoami', 'id', 'uname', 'ls', 'netstat', 'curl', 'ifconfig', 'pwd', 'route']):
        return "singleInjection"
    if any(sym in lower for sym in [';', '&&', '|', '$', '/etc/passwd', 'cat']):
        return "multiInjection"
    return "start"
...
def refine_if_needed(original_payload: str,
    ...
    if grammar_file is None:
        grammar_file = base_dir / "rce" / "rce-grammar.txt"
    if vocab_file is None:
        vocab_file = base_dir / "rce" / "rce_vocab.json"
